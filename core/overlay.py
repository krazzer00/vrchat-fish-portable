...
ROI Overlay
===========
Overlay transparent, click-through, topmost rectangle on game window,
real-time display DETECT_ROI screen position.

- WS_EX_LAYERED + LWA_COLORKEY  → background transparent
...
# Background color (used as transparent key, COLORREF = 0x00BBGGRR)
_KEY_COLOR = 0x00FF00FF           # Magenta, no conflict with green border
# Border color
_BORDER_COLOR = 0x0000FF00        # Green
...
class RoiOverlay:
    ...
    def _run(self):
        ...
        if not self._hwnd:
            log.warning("[Overlay] Failed to create overlay window")
            return
